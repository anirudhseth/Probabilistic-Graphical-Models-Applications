...
odometry = truePose;
for i = 1:size(simSteps, 2)-2
    if(size(simSteps{i}.seenLandmarks,2)==2 && size(simSteps{i+1}.seenLandmarks,2)==2 && size(simSteps{i+2}.seenLandmarks,2)==2) %no of landmarks
        for l=[1:2]
        x1=simSteps{i}.seenLandmarks(1,l);
        y1=simSteps{i}.seenLandmarks(2,l);
        x2=simSteps{i+1}.seenLandmarks(1,l);
        y2=simSteps{i+1}.seenLandmarks(2,l);
        x3=simSteps{i+2}.seenLandmarks(1,l);
        y3=simSteps{i+2}.seenLandmarks(2,l);
        dist=pdist([x1,y1;x2,y2],'euclidean');
        if(dist>1)
%             fprintf('dist is %f and i is %d\n',dist,i);
            simSteps{i+1}.seenLandmarks(1,l)=(simSteps{i}.seenLandmarks(1,l)+simSteps{i+2}.seenLandmarks(1,l))/2;
            simSteps{i+1}.seenLandmarks(2,l)=(simSteps{i}.seenLandmarks(2,l)+simSteps{i+2}.seenLandmarks(2,l))/2;
        end
        end
    end
    if(size(simSteps{i}.seenLandmarks,2)==1 && size(simSteps{i+1}.seenLandmarks,2)==1 && size(simSteps{i+2}.seenLandmarks,2)==1) %no of landmarks
        for l=[1]
        x1=simSteps{i}.seenLandmarks(1,l);
        y1=simSteps{i}.seenLandmarks(2,l);
        x2=simSteps{i+1}.seenLandmarks(1,l);
...
temp=round(numPoses/5,0);
finish=[1,temp*[1:4], numPoses];
