...
temp=round(numPoses/6,0);
finish=[1,temp*[1:5], numPoses];
...
for it = 1:ITERATIONS
    ...
    OmegaRed(:,:,3,3);
    XiRed(:,3);
    %pause  %uncomment to see initial graph
    ...
    if(diff(end)<LeastError)
        LeastError=diff(end);
        LeastIter=it;
    end
    diff(end)
    % Check if it has converged
    if  diff(end) <= CONVERGE
        fprintf('converged')
        if verbose < 2
            [muMap] = graph_slam_solve_map(muRob, Omega, Xi, tau, mapIds);
        end
...
met = mean(errpose(3,:));
display(sprintf('Iterations=%d\nThreshold=%f\nmean error(x, y, theta)=(%f, %f, %f)\nmean absolute error=(%f, %f, %f)\n',ITERATIONS,CONVERGE,mex,mey,met, maex,maey,maet));
