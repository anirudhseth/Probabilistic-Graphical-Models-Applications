...
def fivo(cell,
  ...
  def while_step(t, rnn_state, tas, accs):
    ...
    # Begin Exercise 5: use relative entropy instead of ESS.
    # TODO(rika): refactor and test this
    #log_weights_acc = tf.Print(log_weights_acc,[log_weights_acc],
    #                           message="log_weights_acc", summarize=100)
    main_ent = -tf.reduce_sum(log_weights_acc, axis=0)
    ent = tf.divide(main_ent, num_samples)
    #ent = tf.Print(ent,[ent],message="ent", summarize=100)
    threshold = num_samples
    should_resample = (ent > threshold)
    #should_resample = tf.Print(should_resample,[should_resample],
    #                           message="should_resample", summarize=100)
    # End Exercise 5
